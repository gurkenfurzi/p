...
const elmap={compound:'Pathfinder',title:'Titel',text:'Text',rect:'Rechteck',circle:'Kreis',triangle:'Dreieck',star:'Stern',line:'Linie',arrow:'Pfeil',badge:'Badge',image:'Bild',svg:'SVG',video:'Video/GIF',audio:'Audio',chart:'Diagramm',table:'Tabelle',icon:'Icon',drawing:'Zeichnung',group:'Gruppe'};
...
function defaultProject(){return{appVersion:APP_VERSION,name:'Meine Präsentation',theme:{ink:'#3c312d',accent:'#55c7c4',bg:'#fffdf8'},activeSlideId:null,slides:[]}}
...
function saveProject(){try{localStorage.setItem('slidebloom_v5_project',JSON.stringify(project));localStorage.setItem('slidebloom_v5_media',JSON.stringify(mediaStore));localStorage.setItem('slidebloom_v5_library',JSON.stringify(library));}catch{toast('Browser-Speicher voll. Große Videos/Bilder lieber kleiner verwenden.')}$('#saveStatus').textContent='Gespeichert'}
function loadProject(){try{mediaStore=JSON.parse(localStorage.getItem('slidebloom_v5_media')||localStorage.getItem('slidebloom_media_v1')||'[]');return JSON.parse(localStorage.getItem('slidebloom_v5_project')||localStorage.getItem('slidebloom_ultimate_v1')||'null')}catch{return null}}
function loadLibrary(){try{return JSON.parse(localStorage.getItem('slidebloom_v5_library')||'null')||{textStyles:[],slideTemplates:[],presentationTemplates:[]}}catch{return{textStyles:[],slideTemplates:[],presentationTemplates:[]}}}
function saveLibrary(){try{localStorage.setItem('slidebloom_v5_library',JSON.stringify(library))}catch{}renderTextStyles();renderUserTemplates();renderLibraryManager()}
...
function renderAll(){$('#projectName').value=project.name;renderSlides();renderStage();renderInspector();syncSlideControls();renderLayers();renderTimeline();renderTextStyles();renderUserTemplates();syncMobileSelectionUI()}
...
function renderElement(e,editable=false){
 ...
 if(e.type==='compound'){c.innerHTML=compoundSVG(e);c.classList.add('compound-svg');d.style.background='transparent';d.style.border='none'}
...
function compoundSVG(e){
 const parts=e.parts||[],id='cp_'+String(e.id).replace(/[^a-z0-9]/gi,''),fill=e.fill||project.theme.accent||'#55c7c4',d=parts.map(shapePathD).join(' '),op=e.operation||'union';
...
function buildStoryRail(){
 const side=$('#railSide').value||'left',accent=$('#railAccent').value||'#55c7c4',icons=$('#railIcons').value.split(',').map(x=>x.trim()).filter(Boolean).slice(0,5);while(icons.length<5)icons.push('•');
...
function refreshStoryRail(){if(!project.storyRail)return openModal('#storyRailModal');$('#railSide').value=project.storyRail.side||'left';$('#railAccent').value=project.storyRail.accent||'#55c7c4';$('#railIcons').value=(project.storyRail.icons||[]).join(',');buildStoryRail()}
...
$('#addStoryRailBtn').onclick=()=>{if(project.storyRail){$('#railSide').value=project.storyRail.side||'left';$('#railAccent').value=project.storyRail.accent||'#55c7c4';$('#railIcons').value=(project.storyRail.icons||[]).join(',')}openModal('#storyRailModal')};$('#confirmStoryRailBtn').onclick=buildStoryRail;$('#refreshStoryRailBtn').onclick=refreshStoryRail;
...
renderTemplates();renderIcons();renderAll();setScale(scale,false);history=[JSON.stringify(project)];setTimeout(()=>{fitStage();ensureStageVisible(true)},50);console.info('SlideBloom',APP_VERSION);
