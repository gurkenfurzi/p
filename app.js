...
function stickerHTML(e){
 const kind=e.stickerKind||'sticky',txt=esc(e.content||''),col=e.stickerColor||e.fill||'#fff8ef',acc=e.stickerAccent||'#e8d7a7',sw=Math.max(1,+(e.stickerStrokeWidth||6)),notePattern=e.notePattern||'lined',style=`--sticker-color:${col};--sticker-accent:${acc};--sticker-stroke:${sw};`;
 ...
 if(kind==='pin')return`<div class="sticker-root sticker-pin-ref" style="${style}"><svg viewBox="0 0 120 180" preserveAspectRatio="none"><rect x="52" y="72" width="16" height="88" rx="8" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="60" cy="48" r="34" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="74" cy="34" r="8.5" fill="#fffafc" opacity=".85"/></svg></div>`;
 if(kind==='pushpin')return`<div class="sticker-root sticker-pin-ref" style="${style}"><svg viewBox="0 0 160 220" preserveAspectRatio="none"><g transform="rotate(18 88 110)"><rect x="72" y="126" width="16" height="72" rx="8" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="80" cy="104" r="34" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><path d="M60 88 100 88 92 124 68 124Z" fill="${col}" opacity=".78" stroke="${acc}" stroke-width="${sw}" stroke-linejoin="round"/><ellipse cx="87" cy="56" rx="24" ry="19" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="93" cy="49" r="4.5" fill="#fffafc" opacity=".75"/></g></svg></div>`;
...
function renderInspector(){
 ...
 $('#stickerStrokeWrap')?.classList.toggle('hidden',items.length>1||e.type!=='sticker'||!['title_1','title_2','note_sheet'].includes(e.stickerKind));
