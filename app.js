...
function stickerHTML(e){
 ...
 if(kind==='notebook')return`<div class="sticker-root sticker-notebook sticker-notebook-cute" style="${style}"><svg viewBox="0 0 1248 1248" preserveAspectRatio="none"><rect x="282" y="165" width="741" height="906" rx="40" fill="color-mix(in srgb, ${col} 82%, white)" stroke="${acc}" stroke-width="${sw*4}"/><rect x="250" y="149" width="741" height="906" rx="40" fill="${col}" stroke="${acc}" stroke-width="${sw*4}"/><rect x="346" y="283" width="568" height="200" rx="94" fill="#f3f0f2"/><g fill="none" stroke="${acc}" stroke-width="${sw*4}" stroke-linecap="round" stroke-linejoin="round"><path d="M609 746c-55-57-111-74-139-56-23 15-20 58 8 88 30 30 73 27 131-14Z"/><path d="M639 746c55-57 111-74 139-56 23 15 20 58-8 88-30 30-73 27-131-14Z"/><path d="M579 740c0-22 18-40 45-40s45 18 45 40-18 40-45 40-45-18-45-40Z"/><path d="M595 778c-8 43-29 97-58 139 27-8 44-15 64-28 9-33 15-70 22-111Z"/><path d="M653 778c8 43 29 97 58 139-27-8-44-15-64-28-9-33-15-70-22-111Z"/></g><g stroke="${acc}" stroke-width="${sw*4}" stroke-linecap="round"><line x1="184" y1="296" x2="301" y2="296"/><line x1="184" y1="425" x2="301" y2="425"/><line x1="184" y1="553" x2="301" y2="553"/><line x1="184" y1="682" x2="301" y2="682"/><line x1="184" y1="810" x2="301" y2="810"/><line x1="184" y1="939" x2="301" y2="939"/></g></svg><div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt||'Notes'}</div></div>`;
 ...
 if(kind==='pin')return`<div class="sticker-root sticker-pin" style="${style}"><svg viewBox="0 0 333 696" preserveAspectRatio="none"><rect x="149" y="325" width="33" height="364" rx="17" fill="#edd7ea" stroke="${acc}" stroke-width="${sw*2.1}"/><circle cx="166" cy="166" r="160" fill="${col}" stroke="${acc}" stroke-width="${sw*2.1}"/><circle cx="218" cy="92" r="41" fill="#fef5f9" stroke="${acc}" stroke-width="${sw*2}"/></svg></div>`;
 if(kind==='pushpin')return`<div class="sticker-root sticker-pin" style="${style}"><svg viewBox="0 0 345 612" preserveAspectRatio="none"><rect x="146" y="336" width="38" height="272" rx="19" transform="rotate(16.33 165 472)" fill="#edd7ea" stroke="${acc}" stroke-width="${sw*2.2}"/><ellipse cx="187" cy="273" rx="153" ry="183" transform="rotate(-73.67 187 273)" fill="#ed7396" stroke="#ed7396" stroke-width="1"/><path d="M270 297c-7 4-47 24-95 10-49-14-71-53-75-60l84-285 170 50-84 285Z" fill="color-mix(in srgb, ${col} 82%, white)"/><ellipse cx="241" cy="95" rx="93" ry="126" transform="rotate(-73.67 241 95)" fill="#f195aa" stroke="#ed7396" stroke-width="${sw*2.7}"/><ellipse cx="245" cy="90" rx="93" ry="126" transform="rotate(-73.67 245 90)" fill="${col}"/></svg></div>`;
 ...
 if(kind==='note_sheet'){const W=300,H=220,left=24,top=16,right=20,bottom=20;let pattern='';if(notePattern==='grid'){for(let x=left+12;x<=W-right-8;x+=36)pattern+=`<line x1="${x}" y1="${top+12}" x2="${x}" y2="${H-bottom-8}" stroke="${acc}" stroke-width="${Math.max(1.2,sw*.45)}" opacity=".4"/>`;for(let y=top+18;y<=H-bottom-8;y+=28)pattern+=`<line x1="${left+10}" y1="${y}" x2="${W-right-8}" y2="${y}" stroke="${acc}" stroke-width="${Math.max(1.2,sw*.45)}" opacity=".4"/>`}else{for(let y=top+26;y<=H-bottom-8;y+=34)pattern+=`<line x1="${left+16}" y1="${y}" x2="${W-right-8}" y2="${y}" stroke="${acc}" stroke-width="${Math.max(1.2,sw*.45)}" opacity=".4" stroke-linecap="round"/>`}let holes='';for(let i=0;i<6;i++){const cy=34+i*30;holes+=`<circle cx="12" cy="${cy}" r="7" fill="#000"/><circle cx="12" cy="${cy}" r="12" fill="none" stroke="${acc}" stroke-opacity=".5" stroke-width="${Math.max(1.2,sw*.4)}"/>`}return`<div class="sticker-root sticker-note-sheet ${notePattern==='grid'?'grid':''}" style="${style}"><svg viewBox="0 0 300 220" preserveAspectRatio="none"><rect x="2" y="2" width="296" height="216" rx="2" fill="${col}" stroke="${acc}" stroke-width="${sw}"/>${pattern}${holes}</svg><div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt||'Notiz'}</div></div>`;}
...
const stickerCatalog=[
 ...
 {kind:'note_sheet',name:'Note',cat:'paper',pack:'titles',w:360,h:280,desc:'Referenz-Note, liniert oder karo'},
...
function stickerElement(kind){
 ...
 if(kind==='note_sheet')e.content='Notiz';
