...
function stickerHTML(e){
 const kind=e.stickerKind||'sticky',txt=esc(e.content||''),col=e.stickerColor||e.fill||'#fff8ef',acc=e.stickerAccent||'#e8d7a7',detail=e.stickerDetailColor||'#d9dde6',sw=Math.max(1,+(e.stickerStrokeWidth||6)),notePattern=e.notePattern||'lined',style=`--sticker-color:${col};--sticker-accent:${acc};--sticker-detail:${detail};--sticker-stroke:${sw};`;
 if(kind==='notebook')return`<div class="sticker-root sticker-notebook-ref" style="${style}"><svg viewBox="0 0 320 360" preserveAspectRatio="none"><rect x="64" y="41" width="208" height="281" rx="12" fill="${col}" opacity=".55" stroke="${acc}" stroke-width="${Math.max(2,sw*.7)}"/><rect x="54" y="31" width="208" height="281" rx="12" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><g fill="none" stroke="${acc}" stroke-width="${sw}" stroke-linecap="round">${[86,118,150,182,214].map(x=>`<path d="M${x} 30c0-14 11-25 25-25s25 11 25 25-11 25-25 25-25-11-25-25Z"/>`).join('')}</g><g fill="${acc}" opacity=".65">${[0,1,2,3,4,5,6,7,8].map(r=>[0,1,2,3,4,5,6,7].map(c=>`<circle cx="${96+c*20}" cy="${78+r*24}" r="2.1"/>`).join('')).join('')}</g></svg>${txt?`<div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt}</div>`:''}</div>`;
 ...
 if(kind==='paperclip')return`<div class="sticker-root sticker-paperclip" style="${style}"><svg viewBox="0 0 120 180"><path d="M71 23c19 0 33 13 33 32 0 9-3 17-9 24l-39 50c-11 14-35 8-35-12 0-8 2-14 8-22l35-44c6-8 18-3 18 7 0 4-1 8-4 12L49 106c-4 5-3 11 2 14 5 4 11 3 15-2l25-32" fill="none" stroke="${acc}" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/><circle cx="84" cy="26" r="6" fill="#fff8fb" opacity=".8"/></svg></div>`;
 if(kind==='binder')return`<div class="sticker-root sticker-binder" style="${style}"><svg viewBox="0 0 170 150"><path d="M44 68h82l-7 47H51z" fill="${acc}" stroke="#6a5953" stroke-width="4" stroke-linejoin="round"/><path d="M61 68V41c0-14 10-25 24-25s24 11 24 25v27" fill="none" stroke="#6a5953" stroke-width="7" stroke-linecap="round"/><path d="M56 68c-12-4-19-14-20-24M114 68c12-4 19-14 20-24" fill="none" stroke="#6a5953" stroke-width="4" stroke-linecap="round"/><rect x="60" y="77" width="52" height="9" rx="5" fill="#fff7fb" opacity=".9"/></svg></div>`;
 if(kind==='pin')return`<div class="sticker-root sticker-pin-ref" style="${style}"><svg viewBox="0 0 120 180" preserveAspectRatio="xMidYMid meet"><rect x="52" y="72" width="16" height="88" rx="8" fill="${detail}" stroke="${acc}" stroke-width="${sw}"/><circle cx="60" cy="48" r="34" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="74" cy="34" r="8.5" fill="#fffafc" opacity=".85"/></svg></div>`;
 if(kind==='pushpin')return`<div class="sticker-root sticker-pin-ref" style="${style}"><svg viewBox="0 0 160 220" preserveAspectRatio="xMidYMid meet"><g transform="rotate(18 88 110)"><rect x="72" y="126" width="16" height="72" rx="8" fill="${detail}" stroke="${acc}" stroke-width="${sw}"/><circle cx="80" cy="104" r="34" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><path d="M60 88 100 88 92 124 68 124Z" fill="${detail}" opacity=".95" stroke="${acc}" stroke-width="${sw}" stroke-linejoin="round"/><ellipse cx="87" cy="56" rx="24" ry="19" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><circle cx="93" cy="49" r="4.5" fill="#fffafc" opacity=".75"/></g></svg></div>`;
 ...
 if(kind==='title_1')return`<div class="sticker-root sticker-title1" style="${style}"><svg viewBox="0 0 1015.25 290.71" preserveAspectRatio="none"><path d="M739.58,287.16h268.47s-84.69-96.12-84.69-96.12l88.34-71.92h-269.4c-1.46,56.01-1.26,112.03-2.72,168.04Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round"/><polygon points="828.21 232.49 738.36 287.53 738.69 231.82 828.21 232.49" fill="color-mix(in srgb, ${acc} 35%, ${col})" stroke="${acc}" stroke-width="${sw*.8}" stroke-linecap="round" stroke-linejoin="round"/><path d="M275.66,287.16H7.2l84.69-96.12L3.55,119.12h269.4c1.46,56.01,1.26,112.03,2.72,168.04Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round"/><polygon points="187.04 232.49 276.89 287.53 276.55 231.82 187.04 232.49" fill="color-mix(in srgb, ${acc} 35%, ${col})" stroke="${acc}" stroke-width="${sw*.8}" stroke-linecap="round" stroke-linejoin="round"/><rect x="186.61" y="24.01" width="642.02" height="207.42" fill="${col}" stroke="${acc}" stroke-width="${sw}"/></svg><div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt||'Add Title Here'}</div></div>`;
 if(kind==='title_2')return`<div class="sticker-root sticker-title2" style="${style}"><svg viewBox="1563 0 1012.25 290.71" preserveAspectRatio="none"><path d="M2299.58,286.91h268.47s-84.69-96.12-84.69-96.12l88.34-71.92h-269.4c-1.46,56.01-1.26,112.03-2.72,168.04Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round"/><path d="M2388.21,232.23l-89.85,55.05c.14-20.73.28-41.46.42-62.2l89.44,7.15Z" fill="color-mix(in srgb, ${acc} 35%, ${col})" stroke="${acc}" stroke-width="${sw*.8}" stroke-linecap="round" stroke-linejoin="round"/><path d="M1835.66,286.91h-268.47l84.69-96.12-88.34-71.92h269.4c1.46,56.01,1.26,112.03,2.72,168.04Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linecap="round" stroke-linejoin="round"/><path d="M1747.04,232.23l89.85,55.05c-.11-20.85-.22-41.69-.33-62.54-29.84,2.5-59.68,4.99-89.52,7.49Z" fill="color-mix(in srgb, ${acc} 35%, ${col})" stroke="${acc}" stroke-width="${sw*.8}" stroke-linecap="round" stroke-linejoin="round"/><path d="M2388.63,231.43c-94.1-9.95-198.18-16.52-310.8-16.9-120.61-.4-231.59,6.37-331.22,16.9V24.01c97.23-12.72,208.84-21.45,332.24-20.89,114.33.52,218.28,8.92,309.78,20.89v207.42Z" fill="${col}" stroke="${acc}" stroke-width="${sw}"/></svg><div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt||'Add Title Here'}</div></div>`;
 if(kind==='note_sheet'){let pattern='';if(notePattern==='grid'){for(let x=62;x<=258;x+=28)pattern+=`<line x1="${x}" y1="22" x2="${x}" y2="197" stroke="${acc}" stroke-width="${Math.max(1,sw*.34)}" opacity=".4"/>`;for(let y=22;y<=197;y+=22)pattern+=`<line x1="62" y1="${y}" x2="258" y2="${y}" stroke="${acc}" stroke-width="${Math.max(1,sw*.34)}" opacity=".4"/>`}else{for(let y=32;y<=194;y+=26)pattern+=`<line x1="62" y1="${y}" x2="258" y2="${y}" stroke="${acc}" stroke-width="${Math.max(1,sw*.34)}" opacity=".4" stroke-linecap="round"/>`}let holes='';for(let i=0;i<6;i++){const cy=28+i*36;holes+=`<circle cx="2" cy="${cy}" r="10" fill="#000"/><circle cx="12" cy="${cy}" r="10" fill="none" stroke="${acc}" stroke-width="${Math.max(1,sw*.45)}" opacity=".58"/>`}return`<div class="sticker-root sticker-note-sheet-ref ${notePattern==='grid'?'grid':''}" style="${style}"><svg viewBox="0 0 300 220" preserveAspectRatio="none"><rect x="3" y="3" width="292" height="214" rx="2" fill="${col}" stroke="${acc}" stroke-width="${sw}"/>${pattern}${holes}</svg>${txt?`<div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt}</div>`:''}</div>`;}
 ...
 if(kind==='bow')return`<div class="sticker-root sticker-bow" style="${style}"><svg viewBox="0 0 220 170"><path d="M105 82C74 50 43 41 28 51c-12 8-11 31 4 47 16 16 39 14 73-7Z" fill="${col}" stroke="#5a4a44" stroke-width="4" stroke-linejoin="round"/><path d="M115 82c31-32 62-41 77-31 12 8 11 31-4 47-16 16-39 14-73-7Z" fill="${acc}" stroke="#5a4a44" stroke-width="4" stroke-linejoin="round"/><path d="M89 79c0-12 9-22 21-22s21 10 21 22-9 22-21 22-21-10-21-22Z" fill="#fff8fa" stroke="#5a4a44" stroke-width="4"/><path d="M97 98c-5 16-16 33-28 45l23-9 14-23ZM123 98c5 16 16 33 28 45l-23-9-14-23Z" fill="${col}" stroke="#5a4a44" stroke-width="4" stroke-linejoin="round"/><circle cx="103" cy="73" r="4" fill="#fff" opacity=".7"/></svg></div>`;
 if(kind==='daisy')return`<div class="sticker-root sticker-daisy" style="${style}"><svg viewBox="0 0 180 180"><g fill="#fffef8" stroke="#5a4a44" stroke-width="3.5">${[0,60,120,180,240,300].map(a=>{const r=a*Math.PI/180,cx=90+Math.cos(r)*38,cy=90+Math.sin(r)*38;return `<ellipse cx="${cx}" cy="${cy}" rx="17" ry="28" transform="rotate(${a} ${cx} ${cy})"/>`}).join('')}</g><circle cx="90" cy="90" r="24" fill="${acc}" stroke="#5a4a44" stroke-width="3.5"/><circle cx="82" cy="88" r="2.8" fill="#5a4a44"/><circle cx="98" cy="88" r="2.8" fill="#5a4a44"/><path d="M83 99c4 5 10 5 14 0" fill="none" stroke="#5a4a44" stroke-width="3" stroke-linecap="round"/><circle cx="76" cy="97" r="4" fill="#f8ccd7" opacity=".85"/><circle cx="104" cy="97" r="4" fill="#f8ccd7" opacity=".85"/></svg></div>`;
 ...
 if(kind==='note_top_torn')return`<div class="sticker-root sticker-ui-window" style="${style}"><svg viewBox="0 0 340 180" preserveAspectRatio="none"><path d="M6 16h300c15 0 27 12 27 27v114c-12-5-24-8-36-3-12 4-18 17-28 18-12 2-18-13-30-13-9 0-15 8-23 10-12 3-18-7-30-8-13-1-23 8-36 9-14 1-24-7-38-8-15-1-28 8-42 8-13 0-21-7-34-6-11 0-22 6-30 10V43C-21 28-9 16 6 16Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linejoin="round"/><g fill="#000">${[30,70,110,150,190,230,270].map(x=>`<ellipse cx="${x}" cy="45" rx="4.7" ry="6.5" transform="rotate(24 ${x} 45)"/>`).join('')}</g><g fill="none" stroke="${acc}" stroke-width="${Math.max(1,sw*.8)}">${[30,70,110,150,190,230,270].map(x=>`<circle cx="${x}" cy="45" r="9" opacity=".58"/>`).join('')}</g></svg></div>`;
 if(kind==='window_square')return`<div class="sticker-root sticker-ui-window" style="${style}"><svg viewBox="0 0 250 220" preserveAspectRatio="none"><rect x="26" y="17" width="202" height="183" rx="10" fill="${col}" opacity=".6" stroke="${acc}" stroke-width="${sw}"/><rect x="17" y="8" width="202" height="183" rx="10" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><rect x="28" y="42" width="180" height="135" rx="4" fill="#fffafc" opacity=".9"/><rect x="175" y="18" width="13" height="13" rx="3" fill="#fffafc"/><rect x="193" y="18" width="13" height="13" rx="3" fill="#fffafc"/><path d="M194 24h10" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/><path d="M177 24h9" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/></svg></div>`;
 if(kind==='window_wide')return`<div class="sticker-root sticker-ui-window" style="${style}"><svg viewBox="0 0 350 180" preserveAspectRatio="none"><rect x="15" y="12" width="320" height="150" rx="12" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><rect x="28" y="48" width="292" height="99" rx="3" fill="#fffafc" opacity=".9"/><rect x="263" y="20" width="14" height="14" rx="3" fill="#fffafc"/><rect x="284" y="20" width="14" height="14" rx="3" fill="#fffafc"/><path d="M267 27h7" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/><path d="M287 27h7" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/><path d="M307 23l8 8M315 23l-8 8" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/></svg></div>`;
 if(kind==='folder_window')return`<div class="sticker-root sticker-ui-window" style="${style}"><svg viewBox="0 0 350 220" preserveAspectRatio="none"><rect x="20" y="24" width="300" height="177" rx="10" fill="${col}" opacity=".58" stroke="${acc}" stroke-width="${sw}"/><path d="M22 44h52l17-20h58c9 0 16 7 16 16v16H22Z" fill="${col}" stroke="${acc}" stroke-width="${sw}" stroke-linejoin="round"/><path d="M24 42h299c9 0 17 8 17 17v124c0 9-8 17-17 17H24c-9 0-17-8-17-17V59c0-9 8-17 17-17Z" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><rect x="38" y="60" width="190" height="18" rx="9" fill="#fffafc" opacity=".9"/><rect x="250" y="54" width="15" height="15" rx="3" fill="#fffafc"/><rect x="272" y="54" width="15" height="15" rx="3" fill="#fffafc"/><rect x="294" y="54" width="15" height="15" rx="3" fill="#fffafc"/><rect x="34" y="86" width="281" height="96" rx="3" fill="#fffafc" opacity=".84"/></svg></div>`;
 if(kind==='paint_window')return`<div class="sticker-root sticker-ui-window" style="${style}"><svg viewBox="0 0 300 240" preserveAspectRatio="none"><rect x="18" y="18" width="264" height="205" rx="8" fill="${col}" opacity=".6" stroke="${acc}" stroke-width="${sw}"/><rect x="10" y="10" width="264" height="205" rx="8" fill="${col}" stroke="${acc}" stroke-width="${sw}"/><text x="28" y="30" fill="${acc}" font-family="Nunito, sans-serif" font-weight="700" font-size="18">Paint</text><rect x="215" y="18" width="14" height="14" rx="3" fill="#fffafc"/><rect x="234" y="18" width="14" height="14" rx="3" fill="#fffafc"/><rect x="253" y="18" width="14" height="14" rx="3" fill="#fffafc"/><rect x="24" y="48" width="56" height="56" rx="5" fill="#fffafc" opacity=".9" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/><path d="M42 88c7-18 15-30 31-37 8 4 9 15 1 21-5 4-9 11-10 17-7 1-14 1-22-1Z" fill="${acc}" opacity=".7"/><rect x="24" y="112" width="56" height="72" rx="4" fill="#fffafc" opacity=".9" stroke="${acc}" stroke-width="${Math.max(1,sw*.6)}"/><path d="M52 112v72M24 136h56M24 160h56" stroke="${acc}" stroke-width="${Math.max(1,sw*.5)}" opacity=".7"/><rect x="92" y="48" width="14" height="104" rx="2" fill="#fffafc" opacity=".92" stroke="${acc}" stroke-width="${Math.max(1,sw*.55)}"/><path d="M99 53v16M99 73v16M99 93v16" stroke="${acc}" stroke-width="${Math.max(1,sw*.48)}" opacity=".7"/><rect x="110" y="48" width="140" height="104" rx="3" fill="${col}" opacity=".4" stroke="${acc}" stroke-width="${Math.max(1,sw*.55)}"/><rect x="110" y="154" width="140" height="20" rx="2" fill="#fffafc" opacity=".9" stroke="${acc}" stroke-width="${Math.max(1,sw*.55)}"/><path d="M126 154v20M146 154v20M166 154v20M186 154v20M206 154v20M226 154v20" stroke="${acc}" stroke-width="${Math.max(1,sw*.45)}" opacity=".7"/><rect x="12" y="184" width="260" height="24" rx="3" fill="#fffafc" opacity=".9"/></svg></div>`;
if(kind==='browser_panel')return`<div class="sticker-root sticker-ui-browser" style="${style}"><div class="ui-top"><span></span><span></span><span></span></div><div class="ui-tab">untitled</div><div class="ui-address"></div><div class="ui-scene"><i class="sun"></i><i class="cloud c1"></i><i class="cloud c2"></i><i class="cloud c3"></i></div></div>`;
 if(kind==='search_bar')return`<div class="sticker-root sticker-ui-search" style="${style}"><div class="search-icon">⌕</div><div class="search-text">${txt||'search'}</div><div class="search-pin"></div></div>`;
 if(kind==='folder_cute')return`<div class="sticker-root sticker-ui-folder" style="${style}"><svg viewBox="0 0 260 180"><path d="M28 58h72l18-18h40c11 0 20 9 20 20v12H28z" fill="color-mix(in srgb,${acc} 45%,white)" stroke="#8c5b4e" stroke-width="5" stroke-linejoin="round"/><rect x="22" y="58" width="216" height="96" rx="20" fill="${col}" stroke="#8c5b4e" stroke-width="5"/><path d="M36 84h188" stroke="#fff8" stroke-width="7" stroke-linecap="round" opacity=".65"/></svg></div>`;
 if(kind==='doc_open')return`<div class="sticker-root sticker-ui-doc" style="${style}"><svg viewBox="0 0 170 220"><path d="M34 12h72l30 30v150c0 9-7 16-16 16H34c-9 0-16-7-16-16V28c0-9 7-16 16-16Z" fill="${col}" stroke="#8c5b4e" stroke-width="5"/><path d="M106 12v32h30" fill="none" stroke="#8c5b4e" stroke-width="5" stroke-linejoin="round"/><text x="84" y="126" text-anchor="middle" font-size="42" fill="${acc}" font-family="Nunito, sans-serif" font-weight="800">Aa</text></svg></div>`;
 if(kind==='mail_cute')return`<div class="sticker-root sticker-ui-mail" style="${style}"><svg viewBox="0 0 240 180"><rect x="22" y="34" width="196" height="120" rx="20" fill="${col}" stroke="#8c5b4e" stroke-width="5"/><path d="M28 46 120 114 212 46" fill="none" stroke="#8c5b4e" stroke-width="5" stroke-linejoin="round"/><path d="M28 146 94 90M212 146 146 90" fill="none" stroke="#8c5b4e" stroke-width="5"/><circle cx="192" cy="62" r="18" fill="${acc}" stroke="#8c5b4e" stroke-width="4"/><text x="192" y="68" text-anchor="middle" font-size="16" fill="#fffaf7" font-weight="800">1</text></svg></div>`;
 if(kind==='playlist_cute')return`<div class="sticker-root sticker-ui-playlist" style="${style}"><div class="playlist-title">playlist ♫</div><div class="playlist-line"></div><div class="playlist-row"><b>◁</b><b>▮▮</b><b>▷</b></div><div class="playlist-line"></div><div class="playlist-row"><b>◁</b><b>▮▮</b><b>▷</b></div><div class="playlist-line"></div><div class="playlist-row"><b>◁</b><b>▮▮</b><b>▷</b></div></div>`;
 if(kind==='mini_calendar')return`<div class="sticker-root sticker-ui-calendar" style="${style}"><div class="cal-head"></div><div class="cal-grid">${'<i></i>'.repeat(25)}</div></div>`;
 if(kind==='star_bubble')return`<div class="sticker-root sticker-ui-starbubble" style="${style}"><svg viewBox="0 0 220 170"><path d="M108 20 122 48 154 50 130 71 138 103 108 87 78 103 86 71 62 50 94 48Z" fill="${acc}" stroke="#8c5b4e" stroke-width="4"/><path d="M42 97h118c12 0 22 10 22 22v8c0 12-10 22-22 22H104l-21 16 5-16H42c-12 0-22-10-22-22v-8c0-12 10-22 22-22Z" fill="${col}" stroke="#8c5b4e" stroke-width="4"/></svg></div>`;
...
function stickerKeepsAspect(e){return e?.type==='sticker'&&['pin','pushpin','notebook','window_square','window_wide','folder_window','paint_window'].includes(e.stickerKind)}
...
function renderInspector(){
 ...
 $('#stickerStrokeWrap')?.classList.toggle('hidden',items.length>1||e.type!=='sticker'||!['title_1','title_2','note_sheet','notebook','pin','pushpin','note_top_torn','window_square','window_wide','folder_window','paint_window'].includes(e.stickerKind));
 $('#stickerDetailWrap')?.classList.toggle('hidden',items.length>1||e.type!=='sticker'||!['pin','pushpin'].includes(e.stickerKind));
 ...
 if(e.type==='sticker'){set('#stickerTextInput',e.content||'');set('#stickerColor',safeColor(e.stickerColor||e.fill||'#fff8ef'));set('#stickerAccentColor',safeColor(e.stickerAccent||'#e8d7a7'));set('#stickerTextX',e.stickerTextX||0);set('#stickerTextY',e.stickerTextY||0);set('#stickerStrokeWidth',e.stickerStrokeWidth||6);set('#notePattern',e.notePattern||'lined');set('#milestoneNumber',e.milestoneNumber||'01');if(timelineKinds.includes(e.stickerKind)){set('#timelinePoints',e.timelinePoints||4);set('#timelineStart',e.timelineStart??0);set('#timelineStep',e.timelineStep??1);set('#timelineUnit',e.timelineUnit||'Jahre');set('#timelineLabels',(e.timelineLabels||'').replace(/\|/g,'\n'))}}
...
const stickerCatalog=[
 ...
 {kind:'title_1',name:'Title 1',cat:'decor',pack:'titles',w:470,h:150,desc:'Ribbon wie im Referenzbild'},
 {kind:'title_2',name:'Title 2',cat:'decor',pack:'titles',w:500,h:160,desc:'Gebogenes Ribbon wie im Referenzbild'},
 {kind:'note_sheet',name:'Note',cat:'paper',pack:'titles',w:360,h:280,desc:'Referenz-Notizblatt'},
 {kind:'note_top_torn',name:'Paper Strip',cat:'paper',pack:'titles',w:340,h:180,desc:'Oben gelochter Papierstreifen'},
 {kind:'window_square',name:'Window Square',cat:'decor',pack:'ui',w:250,h:220,desc:'Kleines Fenster wie im Referenzbild'},
 {kind:'window_wide',name:'Window Wide',cat:'decor',pack:'ui',w:350,h:180,desc:'Breites Fenster wie im Referenzbild'},
 {kind:'folder_window',name:'Folder Window',cat:'decor',pack:'ui',w:350,h:220,desc:'Ordner-Fenster wie im Referenzbild'},
 {kind:'paint_window',name:'Paint',cat:'decor',pack:'ui',w:300,h:240,desc:'Paint-Fenster wie im Referenzbild'},
 ...
 {kind:'browser_panel',name:'Cute Browser',cat:'decor',pack:'ui',w:360,h:240,desc:'Retro Browser Panel'},
 {kind:'search_bar',name:'Search Bar',cat:'decor',pack:'ui',w:280,h:78,desc:'Kawaii Suchleiste'},
 {kind:'folder_cute',name:'Cute Folder',cat:'decor',pack:'ui',w:210,h:145,desc:'Dateiordner'},
 {kind:'doc_open',name:'Open Document',cat:'decor',pack:'ui',w:150,h:190,desc:'Dokument-Sticker'},
 {kind:'mail_cute',name:'Mail',cat:'decor',pack:'ui',w:210,h:155,desc:'Süßer Briefumschlag'},
 {kind:'playlist_cute',name:'Playlist Card',cat:'decor',pack:'ui',w:220,h:260,desc:'Musik-Playlist'},
 {kind:'mini_calendar',name:'Mini Calendar',cat:'decor',pack:'ui',w:190,h:170,desc:'Kleiner Kalender / Tracker'},
 {kind:'star_bubble',name:'Star Bubble',cat:'decor',pack:'ui',w:190,h:145,desc:'Stern-Sprechblase'}
];
function stickerElement(kind){
 const def=stickerCatalog.find(s=>s.kind===kind)||stickerCatalog[0],timelineKind=['ruler','timeline_horizontal','timeline_vertical','milestone_card'].includes(kind),e={...shapeEl('sticker',uid('morph'),180,140,def.w,def.h,'transparent',0),stickerKind:kind,stickerColor:'#fff8ef',stickerAccent:timelineKind?(project.theme.accent||'#ef9e9c'):['tape','corner_tape'].includes(kind)?'#e8d7a7':'#d9b7b7',content:'',cropZoom:100,cropX:50,cropY:50,stickerTextX:0,stickerTextY:0,stickerStrokeWidth:6,stickerDetailColor:'#d9dde6',notePattern:'lined',milestoneNumber:'01'};if(['title_1','title_2','note_sheet','note_top_torn','window_square','window_wide','folder_window','paint_window','pin','pushpin'].includes(kind)){e.stickerColor='#f4acc5';e.stickerAccent='#c8568e'}
 if(kind==='notebook'){e.stickerColor='#fffafc';e.stickerAccent='#d56d98'}if(timelineKind){e.timelinePoints=4;e.timelineStart=0;e.timelineStep=1;e.timelineUnit='Jahre';e.timelineLabels=''}
 ...
 if(kind==='title_1'||kind==='title_2')e.content='Add Title Here';
 if(kind==='note_sheet'||kind==='notebook'||kind==='note_top_torn'||kind==='window_square'||kind==='window_wide'||kind==='folder_window')e.content='';
 if(kind==='paint_window')e.content='Paint';
...
$('#stickerAccentColor').oninput=e=>liveSelected(x=>{if(x.type==='sticker')x.stickerAccent=e.target.value});$('#stickerAccentColor').onchange=()=>{commit();renderSlides()};$('#stickerDetailColor').oninput=e=>liveSelected(x=>{if(x.type==='sticker')x.stickerDetailColor=e.target.value});$('#stickerDetailColor').onchange=()=>{commit();renderSlides()};$('#stickerTextX').oninput=e=>liveSelected(x=>{if(x.type==='sticker')x.stickerTextX=+e.target.value});$('#stickerTextX').onchange=()=>{commit();renderSlides()};$('#stickerTextY').oninput=e=>liveSelected(x=>{if(x.type==='sticker')x.stickerTextY=+e.target.value});$('#stickerTextY').onchange=()=>{commit();renderSlides()};$('#stickerStrokeWidth').oninput=e=>liveSelected(x=>{if(x.type==='sticker')x.stickerStrokeWidth=+e.target.value});$('#stickerStrokeWidth').onchange=()=>{commit();renderSlides()};$('#notePattern').onchange=e=>updateSelected(x=>{if(x.type==='sticker'&&x.stickerKind==='note_sheet')x.notePattern=e.target.value});
