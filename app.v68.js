...
function timelineStickerHTML(e,vertical=false){
 const n=Math.max(2,Math.min(10,+(e.timelinePoints||4))),start=Number.isFinite(+e.timelineStart)?+e.timelineStart:0,step=Number.isFinite(+e.timelineStep)?+e.timelineStep:1,unit=esc(e.timelineUnit||'Jahre');
...
function stickerHTML(e){
 ...
 if(kind==='notebook')return`<div class="sticker-root sticker-notebook" style="${style}"><div class="rings"></div><div class="sticker-text" style="transform:translate(${e.stickerTextX||0}px,${e.stickerTextY||0}px)">${txt||'Notizen\n• Punkt 1\n• Punkt 2'}</div></div>`;
 ...
 if(kind==='torn')return`<div class="sticker-root sticker-torn" style="${style}"><div class="sticker-text">${txt||'Gerissener Zettel'}</div></div>`;
 if(kind==='torn_memo')return`<div class="sticker-root sticker-tornmemo" style="${style}"><div class="sticker-text">${txt||'Notiz'}</div></div>`;
 ...
 if(kind==='paperclip')return`<div class="sticker-root sticker-paperclip" style="${style}"><svg viewBox="0 0 100 180"><path d="M60 20c18 0 31 13 31 31 0 11-4 19-10 27l-35 45c-15 18-44 9-44-18 0-10 3-19 10-28l35-45c8-10 26-4 26 10 0 6-2 11-6 16L38 95c-4 5-5 10 0 14 4 4 10 3 14-2l27-35" fill="none" stroke="${acc}" stroke-width="8" stroke-linecap="round" stroke-linejoin="round"/></svg></div>`;
 if(kind==='binder')return`<div class="sticker-root sticker-binder" style="${style}"><svg viewBox="0 0 160 150"><path d="M50 60h60l-8 64H58z" fill="${acc}" stroke="#5d5753" stroke-width="4"/><path d="M62 58V42c0-14 8-22 18-22s18 8 18 22v16" fill="none" stroke="#5d5753" stroke-width="6" stroke-linecap="round"/><path d="M48 60c-8-3-15-10-17-18M112 60c8-3 15-10 17-18" fill="none" stroke="#5d5753" stroke-width="4" stroke-linecap="round"/></svg></div>`;
 if(kind==='pin')return`<div class="sticker-root sticker-pin" style="${style}"><svg viewBox="0 0 130 170"><path d="M46 40c0-14 11-26 25-26s25 12 25 26c0 15-12 26-25 26S46 55 46 40Z" fill="${acc}" stroke="#805e57" stroke-width="4"/><path d="M71 63v72" stroke="#848484" stroke-width="7" stroke-linecap="round"/><path d="M64 135h14l-7 18z" fill="#6f6f6f"/><circle cx="61" cy="31" r="7" fill="#fff8fb" opacity=".65"/></svg></div>`;
 ...
 if(kind==='bow')return`<div class="sticker-root sticker-bow" style="${style}"><svg viewBox="0 0 220 170"><path d="M102 76C73 48 42 37 25 48 12 56 14 85 34 100c19 15 41 8 69-14Z" fill="${col}" stroke="#5a4a44" stroke-width="4"/><path d="M118 76c29-28 60-39 77-28 13 9 11 36-7 52-18 17-40 10-68-14Z" fill="${acc}" stroke="#5a4a44" stroke-width="4"/><ellipse cx="110" cy="77" rx="20" ry="15" fill="#fffaf5" stroke="#5a4a44" stroke-width="4"/><path d="M93 90c-7 23-22 44-40 57l31-12 16-24ZM127 90c7 23 22 44 40 57l-31-12-16-24Z" fill="${col}" stroke="#5a4a44" stroke-width="4" stroke-linejoin="round"/></svg></div>`;
 if(kind==='daisy')return`<div class="sticker-root sticker-daisy" style="${style}"><svg viewBox="0 0 180 180"><g fill="#fffef8" stroke="#5a4a44" stroke-width="3.5" stroke-linejoin="round">${[0,45,90,135,180,225,270,315].map(a=>{const rad=a*Math.PI/180,cx=90+Math.cos(rad)*38,cy=90+Math.sin(rad)*38;return `<ellipse cx="${cx}" cy="${cy}" rx="16" ry="24" transform="rotate(${a} ${cx} ${cy})"/>`}).join('')}</g><circle cx="90" cy="90" r="22" fill="${acc}" stroke="#5a4a44" stroke-width="3.5"/><circle cx="83" cy="83" r="5" fill="#fff3"/></svg></div>`;
...
function addElement(type){
 ...
 if(type==='hexagon')e={...shapeEl('hexagon',uid('morph'),190,160,250,220,project.theme.accent,0),shapeRoundness:0};
...
function openContextMenu(x,y,id=null){const m=$('#contextMenu');if(!m)return;if(id&&!selectedIds.has(id))selectElement(id,false);const e=selected(),isText=!!e&&['title','text','badge','icon'].includes(e.type),isTimeline=!!e&&e.type==='sticker'&&['ruler','timeline_scale','timeline_horizontal','timeline_vertical','milestone_card'].includes(e.stickerKind);$('#contextTextTools')?.classList.toggle('hidden',!isText);$('#contextTimelineEdit')?.classList.toggle('hidden',!isTimeline);if(isText){set('#contextFont',e.fontFamily||'Nunito');set('#contextFontSize',e.fontSize||28);set('#contextTextColor',safeColor(e.color||'#3c312d'))}m.classList.remove('hidden');m.scrollTop=0;requestAnimationFrame(()=>{const rect=m.getBoundingClientRect(),mw=Math.ceil(rect.width||250),mh=Math.ceil(Math.min(window.innerHeight-16,m.scrollHeight||rect.height||420));m.style.left=Math.max(8,Math.min(x,window.innerWidth-mw-8))+'px';m.style.top=Math.max(8,Math.min(y,window.innerHeight-mh-8))+'px'})}
...
function renderInspector(){
 ...
 if(e.type==='star'){set('#starPoints',e.starPoints||5)}
...
function stickerElement(kind){
 ...
 if(kind==='torn')e.content='Gerissener Zettel';
 if(kind==='torn_memo')e.content='Notiz';
...
function storyRailItemsFromInputs(){const count=clamp(+($('#railCount')?.value||5),2,8);const textItems=($('#railIcons')?.value||'').split(',').map(x=>x.trim()).filter(Boolean).map(v=>({type:'text',value:v}));const imageItems=(storyRailUploads||[]).map(it=>({type:'image',value:it.src,name:it.name||'Bild'}));const items=(imageItems.length?imageItems:textItems).slice(0,count);while(items.length<count)items.push({type:'text',value:'•'});return items}
function renderStoryRailUploadList(){const box=$('#railUploadList');if(!box)return;box.innerHTML='';if(!(storyRailUploads||[]).length){box.innerHTML='<small class="hint">Noch keine Story-Rail-Bilder hochgeladen.</small>';return}storyRailUploads.forEach((it,i)=>{const row=document.createElement('div');row.className='storyrail-upload-item';row.innerHTML=`${/^data:image/.test(it.src)?`<img src="${it.src}" alt="">`:'<div style="width:28px;height:28px;border-radius:8px;display:grid;place-items:center;background:#fff;border:1px solid #eadfd7">🖼</div>'}<span>${esc(it.name||'Datei')}</span><button type="button">Entfernen</button>`;row.querySelector('button').onclick=()=>{storyRailUploads.splice(i,1);renderStoryRailUploadList()};box.appendChild(row)})}
...
function fillStoryRailModal(){const sr=project.storyRail||{};$('#railSide').value=sr.side||'left';$('#railAccent').value=sr.accent||project.theme.accent||'#ef9e9c';const textItems=(sr.items||[]).filter(x=>x.type==='text').map(x=>x.value);$('#railIcons').value=textItems.length?textItems.join(','):'⌂,⌕,▤,☼,✓';$('#railCount').value=(sr.items||[]).length||5;storyRailUploads=(sr.items||[]).filter(x=>x.type==='image').map((x,i)=>({src:x.value,name:x.name||`Bild ${i+1}`}));renderStoryRailUploadList()}
function buildStoryRail(){
 const side=$('#railSide').value||'left',accent=$('#railAccent').value||'#ef9e9c',items=storyRailItemsFromInputs(),count=items.length;
 const railW=96,x=side==='left'?0:W-railW,nodeX=side==='left'?20:W-76,startY=84,endY=H-84,gap=count===1?0:(endY-startY)/(count-1),nodeYs=[...Array(count)].map((_,i)=>startY+i*gap);
 clearStoryRail(true);
 project.slides.forEach((s,i)=>{const active=Math.min(i,count-1);const bg={...shapeEl('rect','storyrail_bg',x,0,railW,H,'#fffefa',0),z:900,locked:true,role:'storyRail',shadow:8,borderWidth:1,borderColor:'#edf0eb'};s.elements.push(bg);
   items.forEach((item,n)=>{const isActive=n===active;if(isActive){const ring={...shapeEl('circle',`storyrail_active_${n}`,nodeX-10,nodeYs[n]-34,74,74,'#ffffff',36),z:901,locked:true,role:'storyRail',shadow:5,borderWidth:5,borderColor:accent};s.elements.push(ring)}
     if(item.type==='image'){const e={...shapeEl('image',`storyrail_node_${n}`,nodeX,nodeYs[n]-24,54,54,'transparent',999),src:item.value,objectFit:'contain',cropZoom:100,cropX:50,cropY:50,z:902,locked:true,role:'storyRail',mask:'circle',animation:{in:'none',out:'none',duration:.2,delay:0,trigger:'auto'}};if(item.name)e.name=item.name;s.elements.push(e)}
     else {const e=textEl('icon',`storyrail_node_${n}`,nodeX,nodeYs[n]-24,54,54,item.value,30,isActive?accent:'#8e8b88',true,'center');Object.assign(e,{z:902,locked:true,role:'storyRail',animation:{in:'none',out:'none',duration:.2,delay:0,trigger:'auto'}});s.elements.push(e)}
   });
   if(i>0){s.transition='morph';s.transitionDuration=Math.max(.55,s.transitionDuration||.7)}
 });
 project.storyRail={side,accent,items};commit();renderAll();closeModals();toast('Story Rail erstellt.')
}
...
$('#pathLineCap').onchange=e=>updateSelected(x=>{if(x.type==='drawing')x.lineCap=e.target.value});$('#pathFillToggle').onchange=e=>updateSelected(x=>{if(x.type==='drawing')x.pathFillEnabled=e.target.checked});$('#polygonSides').oninput=e=>liveSelected(x=>{if(x.type==='polygon')x.corners=+e.target.value});$('#polygonSides').onchange=()=>{commit();renderSlides()};$('#shapeRoundness').oninput=e=>liveSelected(x=>{if(['triangle','star','diamond','hexagon','polygon','speechbubble','burst'].includes(x.type))x.shapeRoundness=+e.target.value});$('#shapeRoundness').onchange=()=>{commit();renderSlides()};$('#starPoints').oninput=e=>liveSelected(x=>{if(x.type==='star')x.starPoints=+e.target.value});$('#starPoints').onchange=()=>{commit();renderSlides()};$('#milestoneNumber').onchange=e=>updateSelected(x=>{if(x.type==='sticker'&&x.stickerKind==='milestone_card')x.milestoneNumber=e.target.value||'01'});
...
$('#contextMenu').addEventListener('wheel',e=>e.stopPropagation(),{passive:true});document.addEventListener('pointerdown',e=>{if(!e.target.closest('#contextMenu'))closeContextMenu()},true);document.addEventListener('click',e=>{if(!e.target.closest('#contextMenu'))closeContextMenu()},true);window.addEventListener('blur',closeContextMenu);window.addEventListener('resize',closeContextMenu);window.addEventListener('scroll',closeContextMenu,true);$('#contextFont').onchange=e=>updateSelected(x=>{if(['title','text','badge','icon'].includes(x.type))x.fontFamily=e.target.value});$('#contextFontSize').onchange=e=>updateSelected(x=>{if(['title','text','badge','icon'].includes(x.type))x.fontSize=clamp(+e.target.value,8,220)});$('#contextTextColor').oninput=e=>liveSelected(x=>{if(['title','text','badge','icon'].includes(x.type))x.color=e.target.value});$('#contextTextColor').onchange=()=>{commit();renderSlides()};
...
$('#appAccentColor').oninput=e=>applyUIAccent(e.target.value);$$('[data-uiaccent]').forEach(b=>b.onclick=()=>applyUIAccent(b.dataset.uiaccent));$('#appSettingsBtn').onclick=()=>{set('#settingsUiAccent',uiAccent);set('#settingsDefaultInk',project.theme.ink||appSettings.defaultInk);set('#settingsDefaultAccent',project.theme.accent||appSettings.defaultAccent);set('#settingsDefaultBg',activeSlide().background||appSettings.defaultBg);openModal('#settingsModal')};$('#saveSettingsBtn').onclick=()=>{appSettings={uiAccent:$('#settingsUiAccent').value,defaultInk:$('#settingsDefaultInk').value,defaultAccent:$('#settingsDefaultAccent').value,defaultBg:$('#settingsDefaultBg').value};saveAppSettings();project.theme.ink=appSettings.defaultInk;project.theme.accent=appSettings.defaultAccent;activeSlide().background=appSettings.defaultBg;set('#appAccentColor',appSettings.uiAccent);commit();renderAll();closeModals();toast('Einstellungen gespeichert.')};
