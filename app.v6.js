...
function shapeClipPath(e){
 ...
 if(e.type==='star')return 'polygon(50% 0%,61% 35%,98% 35%,68% 57%,79% 91%,50% 70%,21% 91%,32% 57%,2% 35%,39% 35%)';
...
function stickerHTML(e){
 ...
 if(kind==='binder')return`<div class="sticker-root sticker-binder" style="${style}"><svg viewBox="0 0 160 150"><path d="M46 58h68l-10 68H56z" fill="${acc}" stroke="#5d5753" stroke-width="4"/><path d="M60 58v-14c0-16 9-26 20-26s20 10 20 26v14" fill="none" stroke="#5d5753" stroke-width="7" stroke-linecap="round"/><path d="M52 58c-13-4-18-12-18-22M108 58c13-4 18-12 18-22" fill="none" stroke="#5d5753" stroke-width="5" stroke-linecap="round"/></svg></div>`;
 ...
 if(kind==='bow')return`<div class="sticker-root sticker-bow" style="${style}"><svg viewBox="0 0 200 150"><path d="M92 67C67 40 39 29 22 42 9 52 14 82 31 96 47 109 68 98 91 79Z" fill="${col}" stroke="#5a4a44" stroke-width="4"/><path d="M108 67c25-27 53-38 70-25 13 10 8 40-9 54-16 13-37 2-60-17Z" fill="${acc}" stroke="#5a4a44" stroke-width="4"/><rect x="82" y="58" width="36" height="32" rx="12" fill="#fffaf5" stroke="#5a4a44" stroke-width="4"/><path d="M84 86 60 136l37-24 3-21ZM116 86l24 50-37-24-3-21Z" fill="${col}" stroke="#5a4a44" stroke-width="4"/></svg></div>`;
 if(kind==='daisy')return`<div class="sticker-root sticker-daisy" style="${style}"><svg viewBox="0 0 180 180"><g fill="#fffef8" stroke="#5a4a44" stroke-width="3.5" stroke-linejoin="round"><path d="M90 32c9 0 18 11 18 24S99 80 90 80 72 69 72 56s9-24 18-24Z"/><path d="M90 100c9 0 18 11 18 24s-9 24-18 24-18-11-18-24 9-24 18-24Z"/><path d="M33 90c0-9 11-18 24-18s24 9 24 18-11 18-24 18-24-9-24-18Z"/><path d="M123 90c0-9 11-18 24-18s24 9 24 18-11 18-24 18-24-9-24-18Z"/><path d="M50 50c6-6 19-5 28 4s10 22 4 28-19 5-28-4-10-22-4-28Z"/><path d="M130 50c6 6 5 19-4 28s-22 10-28 4-5-19 4-28 22-10 28-4Z"/><path d="M50 130c6 6 19 5 28-4s10-22 4-28-19-5-28 4-10 22-4 28Z"/><path d="M130 130c6-6 5-19-4-28s-22-10-28-4-5 19 4 28 22 10 28 4Z"/></g><circle cx="90" cy="90" r="24" fill="${acc}" stroke="#5a4a44" stroke-width="3.5"/></svg></div>`;
...
function renderElement(e,editable=false){
 ...
 else if(e.type==='polygon'){const n=Math.max(3,Math.min(12,e.corners||6));const pts=[...Array(n)].map((_,i)=>{const a=-Math.PI/2+i*2*Math.PI/n;return `${50+46*Math.cos(a)},${50+46*Math.sin(a)}`}).join(' ');c.innerHTML=`<svg viewBox="0 0 100 100" width="100%" height="100%"><polygon points="${pts}" fill="${e.fill}"/></svg>`;d.style.background='transparent';d.style.border='none'}
 else if(e.type==='speechbubble'){c.innerHTML=`<svg viewBox="0 0 140 100" width="100%" height="100%"><path d="M21 10h97c12 0 22 10 22 22v28c0 12-10 22-22 22H74L43 97l8-15H21C9 82 0 72 0 60V32C0 20 9 10 21 10Z" fill="${e.fill}"/></svg>`;d.style.background='transparent';d.style.border='none'}
...
function addElement(type){
 ...
 if(type==='polygon')e={...shapeEl('polygon',uid('morph'),190,160,250,220,project.theme.accent,0),corners:6};
 if(type==='speechbubble')e=shapeEl('speechbubble',uid('morph'),180,170,360,200,project.theme.accent,0);
...
function renderInspector(){
 ...
 if(shapeTypes.includes(e.type)){set('#shapeImageZoom',e.shapeImageZoom||100);set('#shapeImageX',e.shapeImageX??50);set('#shapeImageY',e.shapeImageY??50)}
 if(e.type==='polygon'){set('#polygonSides',e.corners||6)}
 if(e.type==='sticker'){set('#stickerTextInput',e.content||'');set('#stickerColor',safeColor(e.stickerColor||e.fill||'#fff8ef'));set('#stickerAccentColor',safeColor(e.stickerAccent||'#e8d7a7'));set('#stickerTextX',e.stickerTextX||0);set('#stickerTextY',e.stickerTextY||0);if(timelineKinds.includes(e.stickerKind)){set('#timelinePoints',e.timelinePoints||4);set('#timelineStart',e.timelineStart??0);set('#timelineStep',e.timelineStep??1);set('#timelineUnit',e.timelineUnit||'Jahre');set('#timelineLabels',(e.timelineLabels||'').replace(/\|/g,'\n'))}}
...
function buildStoryRail(){
 ...
 project.slides.forEach((s,i)=>{s.elements=s.elements.filter(e=>e.role!=='storyRail');const p=project.slides.length<=1?0:i/(project.slides.length-1),markerY=45+p*585;
   ...
   icons.forEach((ic,n)=>{const isActive=n===Math.min(i,icons.length-1);if(isActive){const ring={...shapeEl('circle','storyrail_active',nodeX-8,nodeYs[n]-33,72,72,accent,36),z:901,locked:true,role:'storyRail',shadow:6,borderWidth:5,borderColor:s.background||'#fffdf8'};s.elements.push(ring)}const e=textEl('icon','storyrail_node_'+n,nodeX,nodeYs[n]-25,55,50,ic,30,isActive?'#ffffff':'#a7aaa5',true,'center');Object.assign(e,{z:902,locked:true,role:'storyRail',animation:{in:'none',out:'none',duration:.2,delay:0,trigger:'auto'}});s.elements.push(e)});
   ...
   const mi=textEl('icon','storyrail_marker_icon',markerX+9,markerY+7,40,40,icons[Math.min(i,icons.length-1)]||'✦',24,'#ffffff',true,'center');Object.assign(mi,{z:905,locked:true,role:'storyRail',animation:{in:'none',out:'none',duration:.2,delay:0,trigger:'auto'}});s.elements.push(mi);
...
function normalizeProject(p){p ||= defaultProject();p.id ||= uid('project');p.createdAt ||= Date.now();p.updatedAt ||= Date.now();p.appVersion=APP_VERSION;p.theme ||= {ink:'#3c312d',accent:'#ef9e9c',bg:'#fffdf8'};p.slides ||= [];if(!p.slides.length)p.slides=[defaultSlide()];p.slides.forEach((s,i)=>{s.id ||= uid('slide');s.name ||= 'Folie '+(i+1);s.background ||= p.theme.bg||'#fffdf8';s.elements ||= [];s.comments ||= [];});if(!p.activeSlideId||!p.slides.some(s=>s.id===p.activeSlideId))p.activeSlideId=p.slides[0].id;return p}
...
$('#toggleLayersBtn').onclick=()=>{$('#inspector').classList.add('hidden');$('.editor-row').classList.add('right-closed');$('#layersPanel').classList.remove('hidden');renderLayers()};$('#closeLayers').onclick=()=>{$('#layersPanel').classList.add('hidden')};$('#openLayersPanel2').onclick=()=>{$('#inspector').classList.add('hidden');$('.editor-row').classList.add('right-closed');$('#layersPanel').classList.remove('hidden');renderLayers();$('#rightDockLabel').textContent='Ebenen';$('#openLayersPanel2').classList.add('active');$('#openInspectorPanel').classList.remove('active')};$('#openInspectorPanel').onclick=()=>{$('.editor-row').classList.remove('right-closed');$('#inspector').classList.remove('hidden');$('#layersPanel').classList.add('hidden');$('#rightDockLabel').textContent='Eigenschaften';$('#openInspectorPanel').classList.add('active');$('#openLayersPanel2').classList.remove('active')};$('#closeRightPanels').onclick=()=>{$('#layersPanel').classList.add('hidden');$('#inspector').classList.add('hidden');$('.editor-row').classList.add('right-closed')};$('#openRightDockBtn').onclick=()=>{$('.editor-row').classList.remove('right-closed');$('#inspector').classList.remove('hidden');$('#layersPanel').classList.add('hidden');$('#rightDockLabel').textContent='Eigenschaften';$('#openInspectorPanel').classList.add('active');$('#openLayersPanel2').classList.remove('active')};$('#selectAllBtn').onclick=()=>{selectedIds=new Set(activeSlide().elements.map(e=>e.id));renderAll()};$('#addCommentBtn').onclick=addComment;$('#saveCommentBtn').onclick=saveComment;$('#clearCommentsBtn').onclick=()=>{activeSlide().comments=[];commit();renderLayers()};
